        ...
        switch (current_state) {
            case STATE_START:
                if (difftime(now, last_state_change) > TIMEOUT) {
                    printf("Watchdog: ERROR - Main program stuck in START state.\n");
            ...
            case STATE_SORT_PREPARE:
                if (difftime(now, last_state_change) > TIMEOUT) {
                    printf("Watchdog: ERROR - Main program stuck in SORT_PREPARE state.\n");
                    current_state = STATE_ERROR;
                    exit(1);
                }
                break;
            ...
            case STATE_FIND_MIN:
                if (difftime(now, last_state_change) > TIMEOUT) {
                    printf("Watchdog: ERROR - Main program stuck in FIND_MIN state.\n");
                    current_state = STATE_ERROR;
                    exit(1);
                }
                break;
            ...
            case STATE_SWAP:
                if (difftime(now, last_state_change) > TIMEOUT) {
                    printf("Watchdog: ERROR - Main program stuck in SWAP state.\n");
                    current_state = STATE_ERROR;
                    exit(1);
                }
                break;
            ...
            case STATE_NEXT_ITERATION:
                if (difftime(now, last_state_change) > TIMEOUT) {
                    printf("Watchdog: ERROR - Main program stuck in NEXT_ITERATION state.\n");
                    current_state = STATE_ERROR;
                    exit(1);
                }
                break;
